...
?>
        <div class="wrap">
            <h2><?php _e('System Settings', 'vk-lock'); ?></h2>
            <table cellspacing="1" cellpadding="2"><tbody>
            <tr><td><?php _e('System', 'vk-lock'); ?></td><td><?php echo php_uname(); ?></td></tr>
            <tr><td><?php _e('PHP Version', 'vk-lock'); ?></td>
                <td><?php echo phpversion(); ?>
                <?php
                if (version_compare('5.2', phpversion()) > 0) {
                    echo '&nbsp;&nbsp;&nbsp;<span style="background-color: #ffcc00;">';
                    _e('(WARNING: This plugin may not work properly with versions earlier than PHP 5.2)', 'vk-lock');
                    echo '</span>';
                }
                ?>
                </td>
            <tr><td><?php _e('MySQL Version', 'vk-lock'); ?></td>
                <td><?php echo $this->getMySqlVersion() ?>
                    <?php
                    echo '&nbsp;&nbsp;&nbsp;<span style="background-color: #ffcc00;">';
                    if (version_compare('5.0', $this->getMySqlVersion()) > 0) {
                        _e('(WARNING: This plugin may not work properly with versions earlier than MySQL 5.0)', 'vk-lock');
                    }
                    echo '</span>';
                    ?>
                </td>
            ...
            <h2><?php echo $this->getPluginDisplayName(); echo ' '; _e('Settings', 'vk-lock'); ?></h2>
            ...
            <?php settings_fields($settingsGroup); ?>
                <style type="text/css">
                    ...
                    table.plugin-options-table tr:nth-child(1) {width: 35%;}
                ...
                if ($optionMetaData != null) {
                    foreach ($optionMetaData as $aOptionKey => $aOptionMeta) {
                        ...
                        ?>
                            <tr valign="top">
                                <th scope="row"><p><label for="<?php echo $aOptionKey ?>"><?php echo esc_attr($displayText) ?></label></p></th>
                                <td>
                ...
                <p class="submit">
                    <input type="submit" class="button-primary"
                           value="<?php _e('Save Changes', 'vk-lock') ?>"/>
                </p>
            </form>
        </div>
<?php
    ...
    protected function createFormControl($aOptionKey, $aOptionMeta, $savedOptionValue) {
        if (is_array($aOptionMeta) && count($aOptionMeta) > 2) { // Drop-down list
            ...
            <p><input type="text" name="<?php echo $aOptionKey ?>" id="<?php echo $aOptionKey ?>"
                      value="<?php echo esc_attr($savedOptionValue) ?>" style="width:99%;"/></p>
            <?php
    ...
    protected function getOptionValueI18nString($optionValue) {
        switch ($optionValue) {
            case 'true':
                return __('true', 'vk-lock');
            case 'false':
                return __('false', 'vk-lock');

            case 'Administrator':
                return __('Administrator', 'vk-lock');
            case 'Editor':
                return __('Editor', 'vk-lock');
            case 'Author':
                return __('Author', 'vk-lock');
            case 'Contributor':
                return __('Contributor', 'vk-lock');
            case 'Subscriber':
                return __('Subscriber', 'vk-lock');
            case 'Anyone':
                return __('Anyone', 'vk-lock');
        }
        return $optionValue;
    }
